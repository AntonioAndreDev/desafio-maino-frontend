<template>
    <div v-if="!loading">
        ...
        <div v-if="error" class="bg-red-900/60 p-6 rounded-lg mb-8 border-l-4 border-red-500">
            <div class="flex items-center">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-red-400 mr-3" fill="none"
        ...
        <div v-if="pokemonDetailsData" class="space-y-12">
            <!-- Sprites Section -->
            <div class="bg-gray-800/50 rounded-2xl md:p-6 p-4 backdrop-blur-sm shadow-xl border border-gray-700">
                <h2 class="text-2xl font-semibold mb-6 text-blue-300">Sprites</h2>
                <div class="flex md:flex-row-reverse flex-col-reverse justify-center">
                    <div v-for="(sprite, key) in pokemonDetailsData.sprites" :key="key">
                        <div
                        ...
                        >
                            <img class="size-44 object-contain filter drop-shadow-lg" :src="sprite"
                                 :alt="`Sprite ${key}`"
                            />
                            <span
            ...
            <!-- Moves Section -->
            <div class="bg-gray-800/50 rounded-2xl p-6 backdrop-blur-sm shadow-xl border border-gray-700">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
                    <div v-for="move in pokemonDetailsData.moves?.slice(0, movesLimit)" :key="move.move.name"
                         class="bg-gray-700/40 px-3 py-2 rounded hover:bg-indigo-800/40 transition-colors">
                        <p class="capitalize text-sm">{{ move.move.name.replace('-', ' ') }}</p>
                    </div>
                </div>

                <div v-if="pokemonDetailsData.moves?.length > 20 && movesLimit <= 20"
            ...
            <div v-if="pokemonEvolutionChain"
                ...
                <h2 class="text-2xl font-semibold mb-6 text-blue-300">Cadeia de Evolução</h2>

                <div class="flex md:flex-row flex-col items-center justify-center gap-4">
                    <div v-for="(evolution, index) in getAllEvolutions(pokemonEvolutionChain.chain)" :key="index"
                         class="flex items-center md:w-fit w-full">
                        <div class="bg-gray-700/60 p-4 rounded-lg text-center md:w-fit w-full">
                            <p class="capitalize font-medium">{{ evolution.species.name }}</p>
                        </div>

                        <div v-if="index < getAllEvolutions(pokemonEvolutionChain.chain).length - 1"
            ...
            <!-- Games Section -->
            <div class="bg-gray-800/50 rounded-2xl p-6 backdrop-blur-sm shadow-xl border border-gray-700">
                <h2 class="text-2xl font-semibold mb-6 text-blue-300">Aparições em Jogos</h2>

                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
                    <div v-for="(game, index) in pokemonDetailsData.game_indices" :key="index"
                         class="bg-gray-700/40 px-3 py-2 rounded-lg text-center hover:bg-indigo-800/40 transition-colors">
                        <p class="capitalize text-sm">{{ game.version.name.replace('-', ' ') }}</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div v-if="loading" class="flex justify-center items-center h-screen">
        <div class="relative">
            <div class="w-20 h-20 border-4 border-blue-300 border-t-blue-500 rounded-full animate-spin"></div>
            <div class="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
                <div class="w-10 h-10 bg-gray-900 rounded-full flex items-center justify-center">
                    <div class="w-8 h-8 bg-white rounded-full border-4 border-gray-900"></div>
                </div>
            </div>
        </div>
